...
class _ExplorePageState extends State<ExplorePage> {
  ...
  List<dynamic> _filterMedicines(List<dynamic> medicines) {
    ...
    if (query.isEmpty || query.length < 3) {
      return medicines.take(20).toList(); // Show first 20 items if no search or search < 3 characters
    } else {
      return medicines.where((medicine) {
        final name = (medicine['name'] ?? '').toLowerCase();
        return name.contains(query);
      }).take(20).toList(); // Show first 20 items that match search
    }
  }
          ...
          TextField(
            ...
            onChanged: (value) {
              if (value.length >= 3) {
                ...
                setState(() {
                  displayedMedicines = allMedicines.take(20).toList();
                });
              }
            },
